 ...
 */
template<int dim>
class BallDomain : public FormulaDomain<dim, 0>
{
private:
 ...
 */
template<int dim>
class SphereFunction : public FormulaDomain<dim, 0, dim+1, 1>
{
private:
  static const int space_dim = dim + 1;
  using base_t = Function<dim, 0, dim+1, 1>;
  using parent_t = FormulaDomain<dim, 0, dim+1, 1>;
 ...
 */
template<int dim>
class CylindricalAnnulus : public FormulaDomain<dim, 0, dim, 1>
{
private:
  using base_t = Function<dim, 0, dim, 1>;
  using parent_t = FormulaDomain<dim, 0, dim, 1>;
