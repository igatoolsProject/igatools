                 ...
                 exc         e)
{
  ...
  switch (handling)
  {
    case abort_on_exception:
      iga::iga_exceptions::internals::abort(e);
      break;
    case abort_nothrow_on_exception:
      iga::iga_exceptions::internals::abort(e, /*nothrow =*/ true);
      break;
...
#define Assert(cond, exc)                                                     \
  {                                                                           \
    if (!(cond))                                                              \
      ...
      issue_error(::iga::iga_exceptions::internals::abort_on_exception,\
...
#define AssertNothrow(cond, exc)                                              \
  {                                                                           \
    if (!(cond))                                                              \
      ::iga::iga_exceptions::internals::                                      \
      issue_error(                                                            \
          ::iga::iga_exceptions::internals::abort_nothrow_on_exception,       \
          __FILE__, __LINE__, __PRETTY_FUNCTION__, #cond, #exc, exc);         \
  }
#else
#define AssertNothrow(cond, exc)                                            \
...
#define AssertThrow(cond, exc)                                                \
  {                                                                           \
    if (__builtin_expect(!(cond), false))                                     \
      ::iga::iga_exceptions::internals::                                      \
      issue_error(::iga::iga_exceptions::internals::throw_on_exception,       \
                  __FILE__, __LINE__, __PRETTY_FUNCTION__, #cond, #exc, exc); \
  }
...
#define DeclExceptionMsg(Exception, defaulttext)                          \
  class Exception :  public iga::ExceptionBase                            \
  {                                                                       \
  public:                                                                 \
 ...
 */
#define DeclException0(Exception0)                                        \
  static iga::ExceptionBase& Exception0 ()
 ...
 */
#define DeclExceptionMsg(Exception, defaulttext)                          \
  static iga::ExceptionBase& Exception ()
 ...
 */
#define DeclException1(Exception1, type1, outsequence)                    \
  static iga::ExceptionBase& Exception1 (type1 arg1) throw (errortext outsequence)
 ...
 */
#define DeclException2(Exception2, type1, type2, outsequence)             \
  static iga::ExceptionBase& Exception2 (type1 arg1, type2 arg2) throw (errortext outsequence)
 ...
 */
#define DeclException3(Exception3, type1, type2, type3, outsequence)      \
  static iga::ExceptionBase& Exception3 (type1 arg1, type2 arg2, type3 arg3) throw (errortext outsequence)
 ...
 */
#define DeclException4(Exception4, type1, type2, type3, type4, outsequence) \
  static iga::ExceptionBase& Exception4 (type1 arg1, type2 arg2, type3 arg3, type4 arg4) throw (errortext outsequence)
 ...
 */
#define DeclException5(Exception5, type1, type2, type3, type4, type5, outsequence) \
  static iga::ExceptionBase& Exception5 (type1 arg1, type2 arg2, type3 arg3, type4 arg4, type5 arg5) throw (errortext outsequence)
...
/**
 * Exception denoting that the cache is not filled.
 */
DeclExceptionMsg(ExcCacheNotFilled,
                 "A piece of code is attempting a use a cache that is not filled.");

/**
 * Exception denoting that a functionality is deprecated and should be removed
 * from the library.
 */
DeclExceptionMsg(ExcDeprecated,
                 "A piece of code is attempting a use a a functionality that is "
                 "deprecated and therefore should be removed from the library.");

/**
 * Exception denoting that a functionality that is implemented but is not tested.
 */
DeclExceptionMsg(ExcNonTested,
                 "A piece of code is attempting a use a a functionality that is "
                 "implemented but is not tested yet.");


/**
 * Exception denoting that a number must be zero but is not.
 *
 ...
 */
DeclException1(ExcNonZero,
               std::complex<double>,
               << "In the current function, we encountered a number "
               << "that is not zero (its value is " << arg1 << " and therefore "
...
DeclException1(ExcNumberNotFinite,
               std::complex<double>,
               << "In a significant number of places, igatools checks that some intermediate "
               ...
               << "code (or something in igatools) divides by zero in a place where this "
...
/**
 * Exception raised if a Not-a-Number is detected.
 *
 * This exception should be used to catch not-a-number results.
 */
DeclExceptionMsg(ExcNotANumber,
                 "In a significant number of places, igatools checks that some intermediate "
                 "value is a valid number (as opposed to plus or minus infinity, or "
                 "NaN/Not a Number). In the current function, we encountered a variable "
                 "that is not (cannot represent) a number and therefore "
                 "violates the current assertion.");
...
DeclExceptionMsg(ExcIO,
                 ...
                 "not readable given its file permissions. Second, igatools "
                 ...
                 "data that igatools does not understand."
                 "\n\n"
...
DeclExceptionMsg(ExcNotImplemented,
                 "You are trying to use functionality in igatools that is "
                 ...
                 "consider providing a patch to the igatools development "
                 "sources (see the igatools website on how to contribute).");
...
DeclExceptionMsg(ExcInternalError,
                 ...
                 "in igatools, not in your own program. Try to come up with "
                 "the smallest possible program that still demonstrates "
                 "the error and contact the igatools mailing lists with it "
                 "to obtain help.");
...
DeclExceptionMsg(ExcZero,
                 "In a check in the code, igatools encountered a zero in "
...
/**
 * The object should be empty before this member
 * function is called.
 */
DeclExceptionMsg(ExcNonEmptyObject,
                 "The object you are trying to access is non-empty but it makes "
                 "no sense to attempt the operation you are trying on a "
                 "non-empty object.");

/**
 * The pointer should be non-null before this member
 * function is called.
 */
DeclExceptionMsg(ExcNullPtr,
                 "The pointer is null. This can be caused by several reasons, for example:"
                 "invalid or missing construction, invalid cast, etc.");


/**
 * This exception is thrown if in a getter function or in a search algorithm
 * there is no object (satisfying a certain criteria) to be returned.
 */
DeclExceptionMsg(ExcNotFound,
                 "Object not found.");

/**
 * Exception denoting that a certain object instance is supposed to be unique and it is not.
 */
DeclExceptionMsg(ExcNonUnique,
                 "An object instance is supposed to be unique owner but it is not.");

/**
 * Exception denoting that a shared pointer is supposed to have a unique owner.
 */
template <class T>
DeclException1(ExcNonUniqueOwner,
               std::shared_ptr<const T>,
               << "The shared_ptr is supposed to have a unique owner but currently "
               "is owned by " << arg1.use_count() << " objects.");


/**
 * This exception is thrown if an error occurred working with an XML document.
 *
...
/**
 * This exception is thrown if a warning occurred working with an XML document.
 *
 * If available, it shown information about the error and the line
 * and column where it occurred.
 */
DeclException3(ExcXMLWarning,
               std::string, int, int,
               << "A warning occurred working with XML document at line "
               << (arg2 > 0 ? std::to_string(arg2) : std::string("UNKNOWN")) << " "
               << "column "
               << (arg3 > 0 ? std::to_string(arg3) : std::string("UNKNOWN")) << ". "
               << "The reported error is:\n\t" << arg1);
 ...
 * A number is too big.
 */
DeclException2(ExcUpperRange,
               int, int,
               << "Number " << arg1 << " must be smaller than or equal "
               << arg2 << ").");

/**
 * A generic exception definition for the ExcUpperRange above.
 */
template <typename T>
DeclException2(ExcUpperRangeType,
               T, T,
               << "Number " << arg1 << " must be smaller than or equal "
               << arg2 << ".");
 ...
 *
 * Typically, this will be an internal error of igatools, because the
...
#define AssertDimension(dim1,dim2) Assert((dim1) == (dim2),       \
                                          iga::ExcDimensionMismatch((dim1),(dim2)))
...
#define AssertIndexRange(index,range) Assert((index) < (range), \
                                             iga::ExcIndexRange((index),0,(range)))
 ...
 */
#define AssertIsFinite(number) Assert(iga::numbers::is_finite(number), \
