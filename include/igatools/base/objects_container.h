 ...
 *
 * To every single type (instantiated class) a @p vector of shared pointer
 * of this type is associated.
 *
 * The class provides methods for inserting and getting objects,
 * and also for querying if an object with a given @p Id number is present.
 *
...
public:
   ...
   *
   * Retrieves an object of type @ref T that has the given unique
   * @ref id number.
   *
   ...
   * with the given unique @ref id.
   *
   * Checks if there is any object contained with the given
   * Unique @ref id for the given type @ref T.
   *
   ...
   *         @p false elsewhere.
   */
  template <class T> bool is_id_present (const Index &id) const;

  /**
   * @brief Checks if there is any object contained with the given unique @ref id.
   *
   * Checks if there is any object, of any type, contained with the given
   * unique @ref id.
   *
   * @param[in] id Unique id to checked.
