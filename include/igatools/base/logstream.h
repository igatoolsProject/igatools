...
public:
  ...
  /**
   * Insert the name for a section on the stack,
   * followed by one indentation level.
   */
  void begin_item(const std::string &text);

  /**
   * Removes one indentation level on the stack
...
private:
   ...
   * the "main" thread).
   */
  const std::stack<std::string> &get_prefixes() const;
...
//   mutable Threads::ThreadLocalStorage<std::stack<std::string> > prefixes;
  ...
  std::shared_ptr<std::ostringstream> outstreams;
...
LogStream::get_stream()
{
  ...
  if (outstreams.get() == 0)
  {
    outstreams.reset(new std::ostringstream);
    outstreams->setf(format_flags_);
    outstreams->precision(precision_);
  }
