               ...
               const bool create_knot_mesh) const
{
    ...
    // Setting up the blocks with its corresponding names.
    unsigned int block_index = 0;
    ...
    // Iterating over all the functions in the container for building the VTK maps.
...
create_control_mesh_grid(const MapFunPtr_<dim, codim> mapping) const
{
    ...
    if (dim == 2 || dim == 3)
    {
        if (unstructured_grid_physical_control_) // Unstructured grid
        {
    ...
    {
        // Note: for 1D grids, only Unstructured grids can be used.
...
get_number_functions() const
{
                            ...
                            [&](const auto & type_and_data_same_dim)
    {
                                ...
                                [&](const auto & type_and_data_same_dim_codim)
        {
            ...
            for (const auto &map_and_name : mappings)
            {
                const auto &map = map_and_name.first;
                if (std::dynamic_pointer_cast<IdentityFunction<dim,dim+codim>>(map) != nullptr)
                    ++n_identity;
                else
                {
                    ++n_not_identity;
                    if (std::dynamic_pointer_cast<IgFunction<dim,0,dim+codim,1>>(map) != nullptr)
                        ++n_ig_functions;
                }
            }
        } // end lambda function on codim
