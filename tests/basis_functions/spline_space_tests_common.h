...
/**
 *  @file
 *  @brief Common code for tests/basis_functions/spline_space_*.cpp
 *  @author martinelli
 *  @date Feb 10, 2016
 */
...
//  auto rep_knots = spline_space.compute_knots_with_repetition(ebt, bdry_knots);
  ...
  out.begin_item("Boundary knots:");
  bdry_knots.print_info(out);
  out.end_item();

  out.begin_item("Repeated knots:");
  knots_with_repetitions.print_info(out);
  out.end_item();
  /*
    out << "Boundary knots:\n";
    for (const auto &v : bdry_knots)
      for (const auto &w : v)
        w.print_info(out);
    out << "Repeated knots:\n";
    for (const auto &v : knots_with_repetitions)
      v.print_info(out);
      //*/
}
