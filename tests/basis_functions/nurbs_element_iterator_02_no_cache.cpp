...
void do_test()
{
    ...
    for (int j = 0 ; element != end_element ; ++j, ++element)
    {
        ...
        auto values = element->evaluate_basis_values_at_points(points) ;
        values.print_info(out) ;
        out << endl ;


        out << "Gradients basis functions:" << endl ;
        auto gradients = element->evaluate_basis_gradients_at_points(points) ;
        gradients.print_info(out) ;
        out << endl ;


        out << "Hessians basis functions:" << endl ;
        auto hessians = element->evaluate_basis_hessians_at_points(points) ;
