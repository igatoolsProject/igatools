...
int main()
{
    ...
    shared_ptr<SparsityPattern> sparsity_pattern(new SparsityPattern(*space_manager));

//  const auto n_basis = space_manager->get_num_dofs();
//  const auto n_lcs   = space_manager->get_num_linear_constraints();



    matrix   = Matrix<LAPack::trilinos>::create(*sparsity_pattern);
    rhs      = Vector<LAPack::trilinos>::create(sparsity_pattern->get_row_dofs());
    solution = Vector<LAPack::trilinos>::create(sparsity_pattern->get_col_dofs());
    ...
    for (auto &space_curr: space_infos)
    {
        ...
        //auto a=space_curr.second;
        //auto aa=space_curr.second.get_space_variant();
        //aa->print_info(out);
        //auto aaa=*aa;
        ///auto aa= std::const_pointer_cast<SpaceManager::SpaceInfo>(space_curr.second).get_space_variant();
        auto curr_space=get<std::shared_ptr<PhySpace>>(space_curr.second.get_space_variant());
//      curr_space->print_info(out);
        //auto b=aa->begin();
        //auto c=aa->end();
        //for(;b!=c;++b){
        //  out<<b->get_num_basis()<<endl;

        //}


        auto elem_quad=QGauss<dim>(2+1);
        ...
        const auto elem_end = curr_space->end();
    ...
    for (auto &space_curr: space_infos)
    {
        ...
        /*auto curr_space_id=curr_space->get_id();
        vector<Index> locdf;
        for(auto &val : values)
        {
            locdf.push_back(val.first);
        }
        auto globdf=space_manager->get_global_dofs(curr_space_id,locdf);
        //int c(0);
        out<<locdf[0]<<endl;
        out<<globdf[0]<<endl;
        for(auto &val : values)
        {
            std::pair<int,Real> temp_pair(globdf[c],val.second);
            swap(val,temp_pair);
            //val.first=globdf[c];
            ++c;
        }
