...
auto
MappingSlice<dim_, codim_>::
build_extended_quadrature(const EvaluationPoints<dim> &quad) const -> EvaluationPoints<dim+1>
{
    const auto points  = quad.get_points();
    const auto weights = quad.get_weights();

    auto ext_quad = EvaluationPoints<dim+1>(
...
void
MappingSlice<dim_, codim_>::
init_element(const ValueFlags flag, const EvaluationPoints<dim> &quad) const
{
