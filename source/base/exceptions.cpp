...
//---------------------------------------------------------------------------
//    $Id: exceptions.cc 25343 2012-03-30 21:44:32Z bangerth $
//    Version: $Name$
//
//    Copyright (C) 1998, 2000, 2001, 2002, 2003, 2005, 2006, 2009, 2012 by the dealii authors
//
//    This file is subject to QPL and may not be  distributed
//    without copyright and license information. Please refer
//    to the file dealii/doc/license.html for the  text  and
...
void ExceptionBase::print_stack_trace(std::ostream &out) const
{
  ...
  if (iga_exceptions::show_stacktrace == false)
    return;
  ...
  while ((frame < n_stacktrace_frames)
         ...
         ((std::string(stacktrace[frame]).find("ExceptionBase") != std::string::npos)
          ||
          (std::string(stacktrace[frame]).find("iga_exceptions") != std::string::npos)))
    ++frame;

  // output the rest
  const int first_significant_frame = frame;
  for (; frame < n_stacktrace_frames; ++frame)
  {
    ...
    const int pos_start = stacktrace_entry.find('('),
...
void ExceptionBase::print_exc_data(std::ostream &out) const
{
  ...
  if (iga_exceptions::abort_on_exception == false) {}
                        ...
                        ExceptionBase &e)
{
  ...
  else if (iga_exceptions::abort_on_exception == true)
    std::abort();
  else
    --n_treated_exceptions;
}
...
void abort()
{
  if (iga_exceptions::abort_on_exception == true)
    std::abort();
}
