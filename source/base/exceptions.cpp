...
void ExceptionBase::print_stack_trace(std::ostream &out) const
{
  ...
  // ExceptionBase or iga_exceptions in their names, as these
...
void ExceptionBase::generate_message() const
{
  ...
  try
  {
    ...
    if (!iga_exceptions::additional_assert_output.empty())
    {
      converter << "--------------------------------------------------------"
                << std::endl
                << iga_exceptions::additional_assert_output
                << std::endl;
    }
...
{
  if (iga::iga_exceptions::abort_on_exception)
  {
...
//    deallog << "Exception: " << exc.get_exc_name() << std::endl;
    ...
    //TODO (martinelli, March 15, 2016):
    //     deal.II uses a global variable deallog for logging, currently igatools doesn't.
    //     Maybe we need to use a global logger as deal.II
    std::cerr << exc.what() << std::endl;
    std::cerr << "Exiting because igatools has not a global logger." << std::endl;
    std::abort();
  }
  else
  {
